...
for file in queries/*.sql; do
    ...
    JSON=$(echo "${RESULT}" | jq -scR '. | split("\n") | map(split(",")) | map(select(. != [])) | map({name: ($base + "." + .[0]), value: .[1] | tonumber, interval: (if . | length > 1 then (.[2] | tonumber) else 60 end), time: $timestamp | tonumber })' --arg timestamp $timestamp --arg base $(basename $file .sql))
